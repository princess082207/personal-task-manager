...
<style>
    ...
    .form-input { width: 100%; box-sizing: border-box; background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 12px 16px; font-size: 14px; color: #1e293b; outline: none; transition: all 0.2s; }
...
<div class="gradient-header">
    ...
    <p>Modify and update your saved task settings</p>
</div>
...
<div class="form-card">
    <form action="{{ route('tasks.update', $task) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="task_name" class="form-input" value="{{ old('task_name', $task->task_name) }}">
            @error('task_name') <div class="error-msg">{{ $message }}</div> @enderror
        </div>

        <div class="form-group">
            <label class="form-label">Description:</label>
            <textarea name="description" rows="4" class="form-input" style="resize: vertical;">{{ old('description', $task->description) }}</textarea>
        </div>

        <div class="form-group">
            <label class="form-label">Due Date:</label>
            <input type="date" name="due_date" class="form-input" value="{{ old('due_date', $task->due_date) }}">
